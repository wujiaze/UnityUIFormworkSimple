 ...
 *           注意点：
 *           1. 一般框架内部的调试信息还是使用Debug，框架外部的调试信息使用Log
 *    Date: 2017
 *    Version: 0.1版本
 *    Modify Recoder: 
 *    
...
namespace SimpleUIFramework
{
    ...
    {
        /* 核心字段 */
        private static List<string> _TempLogArray;            //Log日志缓存数据
        ...
        private static int _LogBufferNumber;             //Log日志缓存容量
        /* 日志文件常量定义  */
        //JSON 配置文件“标签常量”
        ...
        //日志状态常量(部署模式)
        private const string JSON_CONFIG_LOG_STATE_DEVELOP = "Develop"; // 开发模式
        private const string JSON_CONFIG_LOG_STATE_SPECIAL = "Speacial"; // 指定模式
        private const string JSON_CONFIG_LOG_STATE_DEPLOY = "Deploy";   // 部署（发布）模式
        private const string JSON_CONFIG_LOG_STATE_STOP = "Stop";   // 停止模式
        //日志默认路径
        private static string JSON_CONFIG_LOG_DEFAULT_PATH = "Default_Log.txt";
        //日志默认最大容量
        private static int LOG_DEFAULT_MAX_CACITY_NUMBER = 2000;
        //日志缓存默认最大容量
        private static int LOG_DEFAULT_MAX_LOG_BUFFER_NUMBER = 1;
        //日志提示信息
        private static string LOG_ImportTIPS = @"Important ! "; // 重要的
        private static string LOG_WarningTIPS = "Warning "; // 警告
        ...
        static Log()
        {
            ...
            //日志文件路径
            IConfigManager configMgr = new ConfigManagerByJson(UIFrameSysDefine.SYS_PATN_LOG_CONFIG_INFO);

            //PC与编辑器环境下的路径，使用配置文件。
#if UNITY_STANDALONE_WIN || UNITY_EDITOR
            string strPCTruePath = string.Empty;
            if (!string.IsNullOrEmpty(configMgr.AppSetting[JSON_CONFIG_LOG_DRIVE_NAME]))
            {
                strPCTruePath = configMgr.AppSetting[JSON_CONFIG_LOG_DRIVE_NAME] + ":\\" + configMgr.AppSetting[JSON_CONFIG_LOG_PATH] + ".txt";
            }
            else
            {
                strPCTruePath = Application.persistentDataPath + configMgr.AppSetting[JSON_CONFIG_LOG_PATH] + ".txt";
            }
            ...
            {
                _LogBufferNumber = Convert.ToInt32(strLogBufferNumber);
            }
            else
            {
                _LogBufferNumber = LOG_DEFAULT_MAX_LOG_BUFFER_NUMBER;
            }
        }//Log_end(构造函数)
        ...
        public static void Write(string writeFileDate, Level level)
        {
            ...
            //如果日志缓存数量超过指定容量，则清空
            if (_TempLogArray.Count >= _LogMaxCapacity)
            {
                _TempLogArray.Clear();   //清空缓存中的数据
            }
        ...
        private static void AppendDateToFile(string writeFileDate)
        {
            ...
            {
                //调试信息数据追加到缓存集合中
                _TempLogArray.Add(writeFileDate);
            }

            //缓存集合数量超过一定指定数量（"_LogBufferNumber"）,则同步到实体文件中。
            if (_TempLogArray.Count % _LogBufferNumber == 0)
            {
                //同步缓存数据信息到实体文件中。
                SyncLogCatchToFile();
            }
        ...
        {
            if (_TempLogArray != null)
            {
                return _TempLogArray;
            }
            else
            {
                return null;
        ...
        public static void SyncLogCatchToFile()
        {
            ...
            {
                foreach (string item in _TempLogArray)
                {
